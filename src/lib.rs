//! SIE 4B language-server library. See `docs/spec.md` for the format.
//!
//! The library is stateless: every public entry point takes the full source
//! as input. This mirrors the `csvx` design — LSP requests re-parse on demand,
//! which keeps the internal model trivial and fast enough for multi-thousand
//! line files.
...
pub use cp437::{decode_cp437, detect_encoding};
pub use labels::{all_labels, label_info, FieldKind, FieldSpec, LabelSpec};
pub use parser::parse;
pub use semtok::{tokens_for as semantic_tokens, TOKEN_TYPES};
pub use types::{
    Diagnostic, Field, FieldValue, Item, ParseOutput, SemanticToken, SemanticTokenKind,
    Severity, Span,
};
...
/// path. Callers that need UTF-16 columns must do their own fixup; the
/// `sie-lsp` binary uses char counts which is fine for Neovim's default.
